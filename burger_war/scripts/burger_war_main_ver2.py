...
class Commander(smach.State):
    ...
    def __init__(self):
        ...
        self.calc_distance = calc_distance.test_calc_distance()
        self.my_trans = []
    ...
    def execute(self, userdata):
        ...
        try:
            ...
            base_frame_name = "/map"
            my_pos = self.tf_listener.lookupTransform(base_frame_name, source_frame_name, rospy.Time(0))
            self.my_trans = [my_pos[0][0], my_pos[0][1], my_pos[0][2]]
            print(my_pos)
        ...
        if len(self.check_points) > 0:
            if len(self.my_trans) != 0:
                self.check_points = self.calc_distance.remake_target_list(self.my_trans, self.check_points)
